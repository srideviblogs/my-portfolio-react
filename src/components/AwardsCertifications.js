...
const AwardsCertifications = () => {
  ...
  return (
    <section id="awards-certifications" className="py-16 px-6 md:px-28 bg-black text-white max-w-6xl mx-auto">
      <h2 className="text-4xl font-bold mb-10 text-center border-b-4 border-tealcustom inline-block">
          ...
          <ul className="space-y-8">
            {awards.map((award, idx) => (
              <li key={idx} className="bg-gray-900 p-6 rounded-2xl shadow-lg">
                <h3 className="text-xl font-semibold text-tealcustom">{award.title}</h3>
        ...
        {activeTab === "certifications" && (
          <ul className="space-y-8">
            {certifications.map((cert, idx) => (
              <li key={idx} className="bg-gray-900 p-6 rounded-2xl shadow-lg">
                <h3 className="text-xl font-semibold text-tealcustom">{cert.title}</h3>
                <p className="text-gray-400 italic mb-2">{cert.year} - {cert.issuer}</p>
                {cert.link && (
                  <a
                    href={cert.link}
                    target="_blank"
